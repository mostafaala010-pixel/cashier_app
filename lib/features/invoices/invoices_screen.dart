...
class _I extends State<InvoicesScreen> {
  ...
  @override Widget build(BuildContext context){
    double total = 0;
    for(var e in inv){ total += (e['total'] as num).toDouble(); }
    if(inv.isEmpty){
      return Scaffold(
        appBar: AppBar(title: const Text('الفواتير', style:TextStyle(fontWeight:FontWeight.bold)), backgroundColor:const Color(0xFF0F172A), foregroundColor:Colors.white),
        body: const Center(child: Text('لا توجد فواتير بعد\nابدأ البيع من الكاشير', textAlign:TextAlign.center, style:TextStyle(fontSize:16))),
      );
    }
    return Scaffold(
      ...
      body: Column(children:[
        Container(margin:const EdgeInsets.all(16), padding:const EdgeInsets.all(20), decoration:BoxDecoration(gradient: const LinearGradient(colors:[Color(0xFF0F172A), Color(0xFF1E293B)]), borderRadius:BorderRadius.circular(16)), child: Row(mainAxisAlignment:MainAxisAlignment.spaceBetween, children:[Column(crossAxisAlignment:CrossAxisAlignment.start, children:[const Text('اجمالي المبيعات', style:TextStyle(color:Colors.white70)), Text('\$total ر.ي', style: const TextStyle(color:Colors.white, fontSize:28, fontWeight:FontWeight.bold))]), Container(padding:const EdgeInsets.all(12), decoration:BoxDecoration(color:Colors.white.withOpacity(0.1), shape:BoxShape.circle), child: const Icon(Icons.payments, color:Colors.green, size:32))])),
        Expanded(child: ListView.builder(padding:const EdgeInsets.all(12), itemCount:inv.length, itemBuilder:(_,i){
          final it=inv[i];
